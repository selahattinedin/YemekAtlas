    ...
    var body: some View {
        ...
        VStack {
            ZStack(alignment: .top) {
                Image("steak") // Replace with dynamic image if available
                    .resizable()
                    .scaledToFill()
                    .frame(width: UIScreen.main.bounds.width, height: 350)
                    .mask(RoundedRectangle(cornerRadius: 45, style: .continuous))
                    .overlay(
                        LinearGradient(
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                Text("Western") // Change to dynamic category if available
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    if selectedTab == "Malzemeler" {
                        ...
                        // Split the instructions by line break and enumerate over them
                        ForEach(Array(recipe.instructions.components(separatedBy: "\n").enumerated()), id: \.element) { index, step in
                            if !step.isEmpty {
                                VStack(alignment: .leading) {
                                    ...
                                    Text(step) // Adım numarasını ve içeriği direkt olarak göster
                                        .font(.body)
                                }
                                .padding()
                                .frame(maxWidth: .infinity, alignment: .leading)
